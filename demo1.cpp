...
void drawCurve(float k)
{
	glColor3f((int)k%8,(int)k%4,(int)k%2);
	printf("%d\n",(int)k );
...
void draw3dCurve()
{
	double j=0.0+curveRot;
	double color;
	while(j<=(360.0+curveRot))
	{

		glPushMatrix();
		{
			glRotatef(j,1,0,0);
			if((int)(j-curveRot)%45==0)
				color=(j-curveRot)/45;
